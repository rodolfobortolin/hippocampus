...
export type Strings = {
  ...
  /** Whose each thing is: the walkthrough step and the Settings section. */
  owners: {
    title: string; lead: string; settingsTitle: string; settingsNote: string
    client: string; personal: string; none: string
    unsure: string; clientName: string; undo: string
    yourAccount: string; knownClient: (client: string) => string; commits: string; cloned: string
    tickets: (prefixes: string) => string; readOnly: (prefixes: string) => string; namedAfterClient: (client: string) => string
    noOwner: (hours: string) => string
    repos: (n: number, names: string) => string
    scanning: string; reading: (name: string) => string; empty: string; showSettled: (n: number) => string; hideSettled: string
    contextTitle: string; contextNote: string; contextPlaceholder: string; contextSaved: string
  }
  ...
  onboarding: {
    ...
    addFolder: string; removeFolder: (folder: string) => string; alsoFound: (folder: string, n: number) => string; include: string
  ...
  settings: {
    ...
    wakeWord: string; wakeWordNote: string
    ...
    /** The version, and the update waiting to be installed. */
    version: string; updatesNote: string; updateReady: (version: string) => string; updateInstall: string
...
const pt: Strings = {
  ...
  owners: {
    title: 'De quem é cada coisa',
    lead: 'Isto é o que o Hippocampus encontrou nos seus repositórios e no seu histórico, e o que ele acha de cada um. Confirme os que estão marcados; o resto já parece certo, e tudo muda depois em Ajustes.',
    settingsTitle: 'Clientes e pessoal',
    settingsNote: 'o que o rascunho de apontamento considera de cada cliente, seu ou de ninguém. Uma resposta sua vale mais que qualquer regra',
    client: 'cliente', personal: 'pessoal', none: 'não é trabalho',
    unsure: 'confirme', clientName: 'nome do cliente', undo: 'voltar à sugestão',
    yourAccount: 'é a sua conta',
    knownClient: (client) => `mesmo cliente que ${client}`,
    commits: 'você tem commits aqui, mas nenhum ticket diz de quem é',
    cloned: 'só clonado: nenhum commit seu',
    tickets: (prefixes) => `tickets ${prefixes}`,
    readOnly: (prefixes) => `você só leu tickets ${prefixes}; nenhum commit ou pergunta sua cita esses tickets`,
    namedAfterClient: (client) => `está na sua conta, mas o nome lembra ${client}`,
    noOwner: (hours) => `${hours} nas últimas duas semanas sem dono`,
    repos: (n, names) => `${n === 1 ? '1 repositório' : `${n} repositórios`}: ${names}`,
    scanning: 'Olhando seus repositórios…',
    reading: (name) => `lendo ${name}`,
    empty: 'Nada para confirmar por enquanto. Com alguns dias de uso, os lugares sem dono aparecem aqui.',
    showSettled: (n) => `mostrar ${n} já resolvido${n === 1 ? '' : 's'}`,
    hideSettled: 'esconder os resolvidos',
    contextTitle: 'Algo mais que ajude a separar',
    contextNote: 'Com suas palavras. O jev lê isto quando uma janela não tem dono claro.',
    contextPlaceholder: 'Ex.: Watlow e prodapac são clientes que atendo pela Valiantys. O atende é um produto meu.',
    contextSaved: 'guardado',
  },
  onboarding: {
    ...
    codeText: 'Commits, branches e de quem é cada projeto vêm dos repositórios destas pastas. Se o seu código fica em mais de um lugar, inclua cada um.',
    ...
    addFolder: 'adicionar outra pasta', removeFolder: (folder) => `deixar de ler ${folder}`,
    alsoFound: (folder, n) => `${folder} também tem ${n === 1 ? '1 repositório' : `${n} repositórios`}:`, include: 'incluir',
  ...
  settings: {
    ...
    wakeWord: 'ouvir a palavra “Hippocampus”', wakeWordNote: 'abre a voz sem tocar no Mac. O reconhecimento de voz do macOS fica ligado no microfone o dia todo, no próprio Mac, e isso gasta CPU e bateria. Desligado, o atalho e o clique na esfera continuam funcionando',
    ...
    codeFolder: 'pastas de código', codeFolderNote: 'onde ficam os repositórios — commits, branches e donos vêm daqui', introAgain: 'rever a introdução',
    ...
    version: 'versão', updatesNote: 'as atualizações chegam sozinhas, do GitHub, e são instaladas quando o app reinicia', updateReady: (v) => `a ${v} já foi baixada e entra quando o app reiniciar`, updateInstall: 'reiniciar e atualizar',
...
const en: Strings = {
  ...
  owners: {
    title: 'Whose is what',
    lead: 'This is what Hippocampus found in your repositories and your history, and what it makes of each. Confirm the ones marked; the rest already look right, and all of it can change later in Settings.',
    settingsTitle: 'Clients and personal',
    settingsNote: 'what the timesheet draft counts as each client\'s, yours or no one\'s. An answer of yours outranks every rule',
    client: 'client', personal: 'personal', none: 'not work',
    unsure: 'confirm', clientName: 'client name', undo: 'back to the suggestion',
    yourAccount: 'this is your account',
    knownClient: (client) => `same client as ${client}`,
    commits: 'you have commits here, but no ticket says whose it is',
    cloned: 'only cloned: no commit of yours',
    tickets: (prefixes) => `tickets ${prefixes}`,
    readOnly: (prefixes) => `you only read tickets ${prefixes}; none of your commits or questions mention them`,
    namedAfterClient: (client) => `it is under your account, but the name suggests ${client}`,
    noOwner: (hours) => `${hours} in the last two weeks with no owner`,
    repos: (n, names) => `${n === 1 ? '1 repository' : `${n} repositories`}: ${names}`,
    scanning: 'Looking through your repositories…',
    reading: (name) => `reading ${name}`,
    empty: 'Nothing to confirm yet. After a few days of use, places with no owner show up here.',
    showSettled: (n) => `show ${n} already settled`,
    hideSettled: 'hide the settled ones',
    contextTitle: 'Anything else that tells them apart',
    contextNote: 'In your own words. jev reads this when a window has no clear owner.',
    contextPlaceholder: 'E.g.: Watlow and prodapac are clients I serve through Valiantys. atende is a product of my own.',
    contextSaved: 'saved',
  },
  onboarding: {
    ...
    codeText: 'Commits, branches and whose each project is come from the repositories in these folders. If your code lives in more than one place, include each.',
    ...
    addFolder: 'add another folder', removeFolder: (folder) => `stop reading ${folder}`,
    alsoFound: (folder, n) => `${folder} also holds ${n === 1 ? '1 repository' : `${n} repositories`}:`, include: 'include',
  ...
  settings: {
    ...
    wakeWord: 'listen for the word “Hippocampus”', wakeWordNote: 'opens the voice without touching the Mac. macOS speech recognition stays on the microphone all day, on this Mac, and that costs CPU and battery. Off, the shortcut and a click on the sphere still work',
    ...
    codeFolder: 'code folders', codeFolderNote: 'where your repositories live — commits, branches and owners come from here', introAgain: 'see the introduction again',
    ...
    version: 'version', updatesNote: 'updates arrive on their own, from GitHub, and are installed when the app restarts', updateReady: (v) => `${v} has downloaded and goes in when the app restarts`, updateInstall: 'restart and update',
...
const es: Strings = {
  ...
  owners: {
    ...
    lead: 'Esto es lo que Hippocampus encontró en tus repositorios y en tu historial, y lo que opina de cada uno. Confirma los marcados; el resto ya parece correcto, y todo se puede cambiar después en Ajustes.',
    settingsTitle: 'Clientes y personal',
    settingsNote: 'lo que el borrador de horas cuenta como de cada cliente, tuyo o de nadie. Una respuesta tuya pesa más que cualquier regla',
    client: 'cliente', personal: 'personal', none: 'no es trabajo',
    unsure: 'confirma', clientName: 'nombre del cliente', undo: 'volver a la sugerencia',
    yourAccount: 'es tu cuenta',
    knownClient: (client) => `mismo cliente que ${client}`,
    commits: 'tienes commits aquí, pero ningún ticket dice de quién es',
    cloned: 'solo clonado: ningún commit tuyo',
    tickets: (prefixes) => `tickets ${prefixes}`,
    readOnly: (prefixes) => `solo leíste tickets ${prefixes}; ninguno de tus commits o preguntas los menciona`,
    namedAfterClient: (client) => `está en tu cuenta, pero el nombre recuerda a ${client}`,
    noOwner: (hours) => `${hours} en las últimas dos semanas sin dueño`,
    repos: (n, names) => `${n === 1 ? '1 repositorio' : `${n} repositorios`}: ${names}`,
    scanning: 'Revisando tus repositorios…',
    reading: (name) => `leyendo ${name}`,
    empty: 'Nada que confirmar por ahora. Con unos días de uso, los lugares sin dueño aparecen aquí.',
    showSettled: (n) => `mostrar ${n} ya resuelto${n === 1 ? '' : 's'}`,
    hideSettled: 'ocultar los resueltos',
    contextTitle: 'Algo más que ayude a separar',
    contextNote: 'Con tus palabras. jev lo lee cuando una ventana no tiene un dueño claro.',
    contextPlaceholder: 'Ej.: Watlow y prodapac son clientes que atiendo a través de Valiantys. atende es un producto mío.',
    contextSaved: 'guardado',
  },
  onboarding: {
    ...
    codeText: 'Los commits, las ramas y de quién es cada proyecto salen de los repositorios de estas carpetas. Si tu código está en más de un sitio, incluye cada uno.',
    ...
    addFolder: 'añadir otra carpeta', removeFolder: (folder) => `dejar de leer ${folder}`,
    alsoFound: (folder, n) => `${folder} también tiene ${n === 1 ? '1 repositorio' : `${n} repositorios`}:`, include: 'incluir',
  ...
  settings: {
    ...
    wakeWord: 'escuchar la palabra “Hippocampus”', wakeWordNote: 'abre la voz sin tocar el Mac. El reconocimiento de voz de macOS se queda en el micrófono todo el día, en este Mac, y eso gasta CPU y batería. Desactivado, el atajo y el clic en la esfera siguen funcionando',
    ...
    codeFolder: 'carpetas de código', codeFolderNote: 'donde están tus repositorios: commits, ramas y dueños salen de aquí', introAgain: 'ver la introducción otra vez',
    ...
    version: 'versión', updatesNote: 'las actualizaciones llegan solas, desde GitHub, y se instalan cuando la app se reinicia', updateReady: (v) => `la ${v} ya se descargó y entra cuando la app se reinicie`, updateInstall: 'reiniciar y actualizar',
...
const fr: Strings = {
  ...
  owners: {
    title: 'À qui est quoi',
    lead: 'Voici ce que Hippocampus a trouvé dans vos dépôts et votre historique, et ce qu’il en pense. Confirmez ceux qui sont marqués ; le reste semble déjà juste, et tout peut changer ensuite dans Réglages.',
    settingsTitle: 'Clients et personnel',
    settingsNote: 'ce que le brouillon de feuille de temps compte pour chaque client, pour vous ou pour personne. Votre réponse l’emporte sur toute règle',
    client: 'client', personal: 'personnel', none: 'pas du travail',
    unsure: 'à confirmer', clientName: 'nom du client', undo: 'revenir à la suggestion',
    yourAccount: 'c’est votre compte',
    knownClient: (client) => `même client que ${client}`,
    commits: 'vous avez des commits ici, mais aucun ticket ne dit à qui c’est',
    cloned: 'seulement cloné : aucun commit de vous',
    tickets: (prefixes) => `tickets ${prefixes}`,
    readOnly: (prefixes) => `vous n’avez fait que lire les tickets ${prefixes} ; aucun de vos commits ni questions ne les cite`,
    namedAfterClient: (client) => `c’est sur votre compte, mais le nom évoque ${client}`,
    noOwner: (hours) => `${hours} ces deux dernières semaines sans propriétaire`,
    repos: (n, names) => `${n === 1 ? '1 dépôt' : `${n} dépôts`} : ${names}`,
    scanning: 'Examen de vos dépôts…',
    reading: (name) => `lecture de ${name}`,
    empty: 'Rien à confirmer pour l’instant. Après quelques jours, les lieux sans propriétaire apparaissent ici.',
    showSettled: (n) => `afficher ${n} déjà réglé${n === 1 ? '' : 's'}`,
    hideSettled: 'masquer ceux qui sont réglés',
    contextTitle: 'Autre chose qui aide à distinguer',
    contextNote: 'Avec vos mots. jev le lit quand une fenêtre n’a pas de propriétaire clair.',
    contextPlaceholder: 'Ex. : Watlow et prodapac sont des clients que je sers via Valiantys. atende est un produit à moi.',
    contextSaved: 'enregistré',
  },
  onboarding: {
    ...
    codeText: 'Les commits, les branches et à qui est chaque projet viennent des dépôts de ces dossiers. Si votre code est à plusieurs endroits, incluez chacun.',
    ...
    addFolder: 'ajouter un autre dossier', removeFolder: (folder) => `ne plus lire ${folder}`,
    alsoFound: (folder, n) => `${folder} contient aussi ${n === 1 ? '1 dépôt' : `${n} dépôts`} :`, include: 'inclure',
  ...
  settings: {
    ...
    wakeWord: 'écouter le mot « Hippocampus »', wakeWordNote: 'ouvre la voix sans toucher le Mac. La reconnaissance vocale de macOS reste sur le micro toute la journée, sur ce Mac, et cela coûte du processeur et de la batterie. Désactivé, le raccourci et le clic sur la sphère fonctionnent toujours',
    ...
    codeFolder: 'dossiers du code', codeFolderNote: 'là où sont tes dépôts — les commits, les branches et les propriétaires viennent d’ici', introAgain: 'revoir l’introduction',
    ...
    version: 'version', updatesNote: 'les mises à jour arrivent seules, depuis GitHub, et s’installent au redémarrage de l’app', updateReady: (v) => `la ${v} est téléchargée et s’installe au prochain redémarrage`, updateInstall: 'redémarrer et mettre à jour',
...
const de: Strings = {
  ...
  owners: {
    ...
    lead: 'Das hat Hippocampus in deinen Repositories und deinem Verlauf gefunden, und so schätzt es jedes davon ein. Bestätige die markierten; der Rest wirkt schon richtig, und alles lässt sich später in den Einstellungen ändern.',
    settingsTitle: 'Kunden und Privates',
    settingsNote: 'was der Entwurf der Zeiterfassung welchem Kunden, dir oder niemandem zurechnet. Deine Antwort wiegt mehr als jede Regel',
    client: 'Kunde', personal: 'privat', none: 'keine Arbeit',
    unsure: 'bestätigen', clientName: 'Name des Kunden', undo: 'zurück zum Vorschlag',
    yourAccount: 'das ist dein Konto',
    knownClient: (client) => `derselbe Kunde wie ${client}`,
    commits: 'du hast hier Commits, aber kein Ticket sagt, wem es gehört',
    cloned: 'nur geklont: kein Commit von dir',
    tickets: (prefixes) => `Tickets ${prefixes}`,
    readOnly: (prefixes) => `du hast Tickets ${prefixes} nur gelesen; keiner deiner Commits und keine Frage nennt sie`,
    namedAfterClient: (client) => `es liegt in deinem Konto, aber der Name erinnert an ${client}`,
    noOwner: (hours) => `${hours} in den letzten zwei Wochen ohne Zuordnung`,
    repos: (n, names) => `${n === 1 ? '1 Repository' : `${n} Repositories`}: ${names}`,
    scanning: 'Deine Repositories werden durchgesehen…',
    reading: (name) => `lese ${name}`,
    empty: 'Noch nichts zu bestätigen. Nach ein paar Tagen erscheinen hier die Orte ohne Zuordnung.',
    showSettled: (n) => `${n} bereits geklärte zeigen`,
    hideSettled: 'geklärte ausblenden',
    contextTitle: 'Sonst noch etwas, das hilft zu unterscheiden',
    contextNote: 'In deinen Worten. jev liest das, wenn ein Fenster keinen klaren Besitzer hat.',
    contextPlaceholder: 'Z. B.: Watlow und prodapac sind Kunden, die ich über Valiantys betreue. atende ist mein eigenes Produkt.',
    contextSaved: 'gespeichert',
  },
  onboarding: {
    ...
    codeText: 'Commits, Branches und wem jedes Projekt gehört kommen aus den Repositories in diesen Ordnern. Liegt dein Code an mehreren Orten, nimm jeden auf.',
    ...
    addFolder: 'weiteren Ordner hinzufügen', removeFolder: (folder) => `${folder} nicht mehr lesen`,
    alsoFound: (folder, n) => `${folder} enthält auch ${n === 1 ? '1 Repository' : `${n} Repositories`}:`, include: 'aufnehmen',
  ...
  settings: {
    ...
    wakeWord: 'auf das Wort „Hippocampus“ hören', wakeWordNote: 'öffnet die Stimme, ohne den Mac zu berühren. Die Spracherkennung von macOS bleibt den ganzen Tag am Mikrofon, auf diesem Mac, und das kostet CPU und Akku. Aus funktionieren Kurzbefehl und Klick auf die Kugel weiter',
    ...
    codeFolder: 'Code-Ordner', codeFolderNote: 'wo deine Repositories liegen — Commits, Branches und Besitzer kommen von hier', introAgain: 'Einführung noch einmal',
    ...
    version: 'Version', updatesNote: 'Updates kommen von selbst, von GitHub, und werden beim Neustart der App installiert', updateReady: (v) => `${v} ist geladen und kommt beim nächsten Neustart`, updateInstall: 'neu starten und aktualisieren',
