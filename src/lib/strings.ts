...
const pt: Strings = {
  ...
  settings: {
    ...
    wideTools: 'ferramentas ampliadas', wideToolsNote: 'desligado, ele responde só do que foi medido aqui, sem abrir arquivos, rodar comandos nem ir à web', wideToolsOn: 'ligado: ele pode ler arquivos, rodar comandos e buscar na web, e usa os seus servidores MCP. O que você pede por voz mostra comandos e mudanças em arquivos na tela antes de rodar',
...
const en: Strings = {
  ...
  settings: {
    ...
    wideTools: 'wider tools', wideToolsNote: 'off, it answers only from what was measured here, without opening files, running commands or going to the web', wideToolsOn: 'on: it can read files, run commands and search the web, and it uses your MCP servers. What you ask by voice shows commands and file changes on screen before they run',
...
const es: Strings = {
  ...
  settings: {
    ...
    wideTools: 'herramientas ampliadas', wideToolsNote: 'desactivado, responde solo con lo medido aquí, sin abrir archivos, ejecutar comandos ni ir a la web', wideToolsOn: 'activado: puede leer archivos, ejecutar comandos y buscar en la web, y usa tus servidores MCP. Lo que pides por voz muestra los comandos y cambios en archivos en pantalla antes de ejecutarlos',
...
const fr: Strings = {
  ...
  settings: {
    ...
    wideTools: 'outils élargis', wideToolsNote: 'désactivé, il répond seulement avec ce qui a été mesuré ici, sans ouvrir de fichiers, lancer de commandes ni aller sur le web', wideToolsOn: 'activé : il peut lire des fichiers, lancer des commandes et chercher sur le web, et il utilise tes serveurs MCP. Ce que tu demandes à voix haute montre les commandes et les modifications de fichiers à l’écran avant de les lancer',
...
const de: Strings = {
  ...
  settings: {
    ...
    wideTools: 'erweiterte Werkzeuge', wideToolsNote: 'aus antwortet er nur aus dem hier Gemessenen, ohne Dateien zu öffnen, Befehle auszuführen oder ins Web zu gehen', wideToolsOn: 'an: er darf Dateien lesen, Befehle ausführen und im Web suchen und nutzt deine MCP-Server. Was du per Stimme verlangst, zeigt Befehle und Dateiänderungen vorher auf dem Bildschirm',
