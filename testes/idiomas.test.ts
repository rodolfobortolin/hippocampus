...
import { IDIOMAS, COMO_ESCREVER, CATEGORIAS_POR_IDIOMA, NOMES_CATEGORIA, NIVEIS_POR_IDIOMA, PERGUNTAS_JEV, type Idioma } from '../core/idiomas.ts'
import { PERSONAS } from '../core/personas.ts'
import { DOSSIE } from '../core/dossie.ts'
...
test('os cinco idiomas têm as mesmas categorias, com as mesmas chaves', () => {
  const referencia = Object.keys(CATEGORIAS_POR_IDIOMA['pt-BR']).sort()
  for (const idioma of idiomas) {
    assert.deepEqual(Object.keys(CATEGORIAS_POR_IDIOMA[idioma]).sort(), referencia,
      `as categorias de ${idioma} divergem das de pt-BR`)
    for (const chave of referencia) {
      assert.ok(NOMES_CATEGORIA[idioma][chave],
...
test('a categoria sem rótulo tem nome em todo idioma', () => {
  ...
  for (const idioma of idiomas) {
    assert.ok(NOMES_CATEGORIA[idioma]['sem rótulo'], `falta em ${idioma}`)
  }
})

test('cada idioma traz três níveis de complexidade para o jev', () => {
  for (const idioma of idiomas) {
    assert.equal(NIVEIS_POR_IDIOMA[idioma].length, 3,
...
test('o dossiê fala do trabalho delegado em todo idioma', () => {
  ...
  for (const idioma of idiomas) {
    const frase = DOSSIE[idioma].delegado('1h20', '0h30')
